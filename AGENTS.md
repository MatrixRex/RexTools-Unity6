...
- **Release:** When prompted with `release` (or `release major`, `release minor`, `release patch`), follow `.agent/workflows/release.md`. Check current version in `package.json`, bump version according to SemVer, update `package.json`, stamp `CHANGELOG.md` (convert `## [Unreleased]` to `## [X.Y.Z] - YYYY-MM-DD` and insert a new empty `## [Unreleased]`), verify and update `readme.md` for any new/updated tools or features, automatically stage and commit the release with `chore(release): v<NEW_VERSION>` (including any modified `.meta` files), and output release summary.
